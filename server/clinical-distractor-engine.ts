// ============================================================================
// ONE SHOT FMGE — Server-side Clinical Distractor & High-Yield Mnemonic Engine
// ============================================================================
...
export function enrichClinicalQuestionServer(q: {
  stem?: string;
  question?: string;
  ...
  const existing = q.whyOtherOptionsAreWrong || [];
  const isGeneric = existing.some((d) =>
  ...
  let whyOtherOptionsAreWrong = existing;
  if (matched && (existing.length === 0 || isGeneric)) {
    whyOtherOptionsAreWrong = matched.whyOtherOptionsAreWrong;
  } else if (existing.length === 0 || isGeneric) {
