...
export function calculateStudyReadiness(state: AppState): ReadinessBreakdown {
  ...
  let mcqDetails = 'Solve QBanks or Telegram Daily Quizzes to record accuracy.';

  if (accuracyCount > 0 || solvedTelegramQuestions.length > 0) {
    ...
    if (solvedTelegramQuestions.length > 0) {
    ...
    mcqDetails = `Aggregated across ${accuracyCount} topic QBanks and ${solvedTelegramQuestions.length} daily quiz questions.`;
